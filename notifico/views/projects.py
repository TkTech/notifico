...
class ProjectDetailsForm(wtf.FlaskForm):
    name = fields.StringField(
        ...
        description=_("A descriptive name for your project."),
    )
...
class HookDetailsForm(wtf.FlaskForm):
    service_id = fields.SelectField(
        "Service", validators=[validators.InputRequired()], coerce=int
    )
...
class ChannelEditForm(wtf.FlaskForm):
    ...
    public = fields.BooleanField(
        _("Public"),
        default=True,
        description=_("Allow others to see that this channel exists."),
    )
...
class ChannelCreateForm(ChannelEditForm):
    network = fields.SelectField(_("Network"))
...
class ChannelLoggingForm(wtf.FlaskForm):
    enabled = fields.BooleanField(
        _("Enable Logging"),
        default=False,
        description=_("Enable logging for this channel."),
    )
...
class ChannelDeleteForm(wtf.FlaskForm):
    confirm_channel = fields.StringField(
        ...
        description=_("Type the name of the channel to confirm deletion."),
    )
...
class ProjectDeleteForm(wtf.FlaskForm):
    confirm_project = fields.StringField(
        _("Project Name"), validators=[validators.InputRequired()]
    )
...
def dashboard(u: User):
    ...
    user_projects = u.projects.order_by(False).order_by(Project.created.desc())
...
def new():
    ...
    return render_template("projects/new_project.html", form=form, action="new")
    ...
    match request.form.get("action"):
        ...
        case "delete":
            if delete_form.validate_on_submit():
                ...
                flash(_("Your project has been deleted."), category="success")
...
def details(u, p: Project):
    ...
    return render_template(
        ...
        project=p,
        user=u,
        page_title="Notifico! - {u.username}/{p.name}".format(u=u, p=p),
    )


@projects.route(
    "/<u>/<p>/hook/new", defaults={"sid": 10}, methods=["GET", "POST"]
)
...
def new_hook(u, p: Project, sid):
    ...
    if form and hook.validate(form, request):
        ...
        db_session.add(h)
        db_session.commit()
        return render_template("projects/hook_ready.html", project=p, hook=h)
...
def delete_hook(u, p: Project, hid):
    ...
    return render_template("projects/delete_hook.html", project=p, hook=h)
...
def new_channel(u, p: Project):
    ...
    networks = (
        IRCNetwork.only_readable(db_session.query(IRCNetwork))
        .order_by(IRCNetwork.public.asc())
        .all()
    )
    ...
    # shortcuts.
    common_channels = (
        Channel.only_readable(db_session.query(Channel))
        .join(Channel.project)
        .filter(Project.owner_id == g.user.id, Channel.project_id != p.id)
        .distinct(Channel.network_id)
        .order_by(Channel.network_id, Channel.created.desc())
        .limit(3)
        .all()
    )
    ...
    for network in networks:
        if network.public is not None and network.public > 0:
            ...
            form.network.choices.append((network.id, f"{network.host}"))
        elif network.ssl:
            form.network.choices.append(
                (network.id, f"★ {network.host}:{network.port} (Using SSL)")
            )
        else:
            form.network.choices.append(
                (network.id, f"★ {network.host}:{network.port}")
            )
    ...
    if request.method == "POST":
        if request.form.get("action") == "quick-add":
            channel = (
                ...
                .filter(Channel.id == request.form["channel-id"])
                .first()
            )
            ...
            c = Channel.query.filter_by(
                channel=channel, network=network, project_id=p.id
            ).first()
            if not c:
                c = Channel(
                    channel=channel,
...
def edit_channel(u, p: Project, cid):
    ...
    c: Channel = Channel.query.filter_by(id=cid, project_id=p.id).first()
    ...
    logging_form = ChannelLoggingForm(
        prefix="logging", data={"enabled": c.logged}, meta={"channel": c}
    )
        ...
        case "edit":
            if edit_form.validate_on_submit():
                if c.logged:
                    ...
                    db_session.commit()
                    flash(
                        _("The channel has been updated."), category="success"
                    )
                    return redirect(url_for(".details", p=p.name, u=u.username))
        case "delete":
            if delete_form.validate_on_submit():
