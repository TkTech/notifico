...
class NetworkDeleteForm(wtf.FlaskForm):
    confirm_host = fields.StringField(
        lg("Host"), validators=[validators.InputRequired()]
    )
...
class NetworkDetailsForm(wtf.FlaskForm):
    ...
    port = fields.IntegerField(
        lg("Port"),
        validators=[validators.NumberRange(1024, 66552)],
        default=6697,
    )
    ssl = fields.BooleanField(lg("Use SSL"), default=True)
...
def irc_network_new():
    ...
    if form.validate_on_submit():
        if IRCNetwork.can(Action.CREATE):
            ...
            flash(
                lg("Your custom network has been created."), category="success"
            )

        return redirect(url_for(".irc"))

    return render_template(
        "settings/irc_network.html", network_form=form, action="new"
    )
...
def irc_network_edit(network_id: int):
    ...
    delete_form = NetworkDeleteForm(prefix="delete", meta={"network": network})
    delete_form.confirm_host.render_kw = {"placeholder": network.host}
