...
def simplify_payload(payload):
    result = {
        ...
        "tag": None,
        "pusher": None,
        "files": {"all": [], "added": [], "removed": [], "modified": []},
        "original": payload,
    }
...
class GiteaConfigForm(wtf.FlaskForm):
    branches = fields.StringField(
        "Branches",
        validators=[validators.Optional(), validators.Length(max=1024)],
        description=(
    ...
    show_branch = fields.BooleanField(
        "Show Branch Name",
        validators=[validators.Optional()],
        default=True,
        description=(
    ...
    show_tags = fields.BooleanField(
        "Show Tags",
        validators=[validators.Optional()],
        default=True,
        description=("If checked, changes to tags will be shown."),
    )
    prefer_username = fields.BooleanField(
        "Prefer Usernames",
        validators=[validators.Optional()],
        default=True,
        description=(
    ...
    full_project_name = fields.BooleanField(
        "Full Project Name",
        validators=[validators.Optional()],
        default=False,
        description=(
    ...
    title_only = fields.BooleanField(
        "Title Only",
        validators=[validators.Optional()],
        default=False,
        description=(
...
def _create_push_summary(project_name, j, config):
    ...
    line.append(
        "{RESET}[{BLUE}{name}{RESET}]".format(
            name=project_name, **IncomingHookService.colors
        )
    )

    # The user doing the push.
    line.append(
        "{ORANGE}{pusher}{RESET} pushed".format(
            pusher=j["pusher"], **IncomingHookService.colors
        )
    )

    # The number of commits included in this push.
    line.append(
        "{GREEN}{count}{RESET} {commits}".format(
    ...
    if show_branch and j["branch"]:
        line.append(
            "to {GREEN}{branch}{RESET}".format(
                branch=j["branch"], **IncomingHookService.colors
            )
        )

    # File movement summary.
    line.append(
        "[+{added}/-{removed}/\u00B1{modified}]".format(
    ...
    if original["compare_url"]:
        line.append(
            "{PINK}{compare_link}{RESET}".format(
...
def _create_commit_summary(project_name, j, config):
    ...
    for commit in original["commits"]:
        ...
        line.append(
            "{GREEN}{sha}{RESET}".format(
                sha=commit["id"][:7], **IncomingHookService.colors
            )
        )
        ...
        if attribute_to:
            line.append(
                "{ORANGE}{attribute_to}{RESET}".format(
                    attribute_to=attribute_to, **IncomingHookService.colors
                )
            )
            line.append("-")
...
class GiteaHook(EnvironmentMixin, IncomingHookService):
    ...
    def _handle_create(cls, user, request, hook, json):
        fmt_string = " ".join(
            [
                "{RESET}[{BLUE}{name}{RESET}] {ORANGE}{who}{RESET} "
                "created {ref_type}",
                # null/None if repository was created
                "{GREEN}{ref}{RESET}" if json["ref"] else "",
                "- {PINK}{url}{RESET}",
            ]
        )
    ...
    def _handle_delete(cls, user, request, hook, json):
        fmt_string = (
            "{RESET}[{BLUE}{name}{RESET}] {ORANGE}{who}{RESET} deleted "
    ...
    def _handle_fork(cls, user, request, hook, json):
        fmt_string = (
            "{RESET}[{BLUE}{name}{RESET}] {ORANGE}{who}{RESET} forked "
    ...
    def _handle_push(cls, user, request, hook, json):
        ...
        if not original["commits"]:
            if show_tags and j["tag"]:
                yield cls.message(
                    cls._create_non_commit_summary(j, config), strip=strip
                )
            if j["branch"]:
                yield cls.message(
                    cls._create_non_commit_summary(j, config), strip=strip
        ...
        yield cls.message(
            _create_push_summary(project_name, j, config), strip=strip
        )
        ...
        for i, formatted_commit in enumerate(line_iterator):
            if i > line_limit or (i == line_limit and not num_commits == i + 1):
                yield cls.message(
                    _create_push_final_summary(project_name, j, config),
                    strip=strip,
                )
                break
    ...
    def _handle_issues(cls, user, request, hook, json):
        fmt_string = (
            "{RESET}[{BLUE}{name}{RESET}] {ORANGE}{who}{RESET} {action} "
    ...
    def _handle_issue_comment(cls, user, request, hook, json):
        ...
        if (
            event_extended_type == "pull_request_comment"
            and json["action"] == "reviewed"
        ):
            if is_event_allowed(
                hook.config, "pull_request_review_comment", None
            ):
        ...
        event_type = (
            "issue_comment" if not json["is_pull"] else "pull_request_comment"
        )
        ...
        fmt_string = (
            "{RESET}[{BLUE}{name}{RESET}] {ORANGE}{who}{RESET} {action} on "
    ...
    def _handle_pull_request(cls, user, request, hook, json):
        fmt_string = (
            "{RESET}[{BLUE}{name}{RESET}] {ORANGE}{who}{RESET} {action} pull "
        ...
        if j["tag"]:
            if not original.get("head_commit"):
                if not is_event_allowed(config, "delete", "tag"):
                    return ""
                line.append("deleted" if j["pusher"] else "Deleted")
                line.append("tag")
            else:
                if not is_event_allowed(config, "create", "tag"):
                    return ""
                # Verb with proper capitalization
                line.append("tagged" if j["pusher"] else "Tagged")

                # The sha1 hash of the head (tagged) commit.
                line.append(
                    "{GREEN}{sha}{RESET} as".format(
            ...
            # The tag itself.
            line.append(
                "{GREEN}{tag}{RESET}".format(
                    tag=j["tag"], **IncomingHookService.colors
                )
            )
        elif j["branch"]:
            # Verb with proper capitalization
            if original["deleted"]:
                if not is_event_allowed(config, "delete", "branch"):
                    return ""
                line.append(
                    "deleted branch" if j["pusher"] else "Deleted branch"
                )
            else:
                if not is_event_allowed(config, "create", "branch"):
                    return ""
                line.append(
                    "created branch" if j["pusher"] else "Created branch"
                )

            # The branch name
            line.append(
                "{GREEN}{branch}{RESET}".format(
                    branch=j["branch"], **IncomingHookService.colors
                )
            )
