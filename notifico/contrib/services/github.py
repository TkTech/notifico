...
class GithubConfigForm(wtf.FlaskForm):
    ...
    events = EventSelectField(
        ...
        choices=[
            ...
            (
                "check_run_conclusion_cancelled",
                "Check Run conclusion: cancelled",
            ),
            (
                "check_run_conclusion_timed_out",
                "Check Run conclusion: timed out",
            ),
            (
    ...
    distinct_only = fields.BooleanField(
...
def _create_push_summary(project_name, j, config):
    ...
    line.append(
        "{PINK}{compare_link}{RESET}".format(
            compare_link=original["compare"], **IncomingHookService.colors
        )
    )
...
class GithubHook(EnvironmentMixin, IncomingHookService):
    """
    EnvironmentMixin hook for https://github.com.
    """
    ...
    def _handle_ping(cls, user, request, hook, json):
        yield "{RESET}[{BLUE}GitHub{RESET}] {zen}".format(
            zen=json["zen"], **IncomingHookService.colors
        )
    ...
    def _handle_pull_request_review_comment(cls, user, request, hook, json):
        fmt_string = (
            "{RESET}[{BLUE}{name}{RESET}] {ORANGE}{who}{RESET} reviewed pull "
    ...
    def _handle_gollum(cls, user, request, hook, json):
        ...
        if len(json["pages"]) > 1:
            ...
            fmt_string_page = (
                "{RESET}[{BLUE}{name}{RESET}] wiki page {GREEN}{pname}{RESET}"
    ...
    def _handle_watch(cls, user, request, hook, json):
        fmt_string = (
            "{RESET}[{BLUE}{name}{RESET}] {ORANGE}{who}{RESET} starred "
    ...
    def _handle_public(cls, user, request, hook, json):
        fmt_string = (
            "{RESET}[{BLUE}{name}{RESET}] {ORANGE}{who}{RESET} made the "
            "repository public!"
        )
    ...
    def _handle_team_add(cls, user, request, hook, json):
        fmt_string = (
            "{RESET}[{BLUE}{name}{RESET}] {ORANGE}{who}{RESET} added the"
    ...
    def _handle_status(cls, user, request, hook, json):
        fmt_string = (
            "{RESET}[{BLUE}{name}{RESET}] {status_color}{status}{RESET}. "
    ...
    def _handle_check_run(cls, user, request, hook, json):
        conclusion_color = IncomingHookService.colors["GREEN"]
        conclusion = json["check_run"]["conclusion"]
        if conclusion is not None:
            if not conclusion == "success":
                conclusion_color = IncomingHookService.colors["RED"]
            conclusion = conclusion.capitalize()
            fmt_string = (
                "{RESET}[{BLUE}{name}{RESET}] Check Run for "
