...
class BitbucketConfigForm(wtf.FlaskForm):
    ...
    show_raw_author = fields.BooleanField(
        "Show Raw Author",
        validators=[validators.Optional()],
        default=False,
        description=(
...
def _make_summary_line(hook, j, config):
    ...
    line.append(
        "{RESET}[{BLUE}{name}{RESET}]".format(
            name=original["repository"]["name"], **IncomingHookService.colors
        )
    )
    ...
    line.append(
        "{PINK}{0}{RESET}".format(
            BitbucketHook.shorten(link), **IncomingHookService.colors
        )
    )
...
def _make_commit_line(hook, j, commit):
    ...
    line.append(
        "{GREEN}{0}{RESET}".format(
            commit["node"][:7], **IncomingHookService.colors
        )
    )
...
class BitbucketHook(EnvironmentMixin, IncomingHookService):
