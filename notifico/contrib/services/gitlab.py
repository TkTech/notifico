...
def action_filter(category, action_key="action"):
    def decorator(f):
        ...
        def wrapper(cls, user, request, hook, json):
            event = (
                json["object_attributes"][action_key] if action_key else None
            )
...
def _create_push_summary(project_name, j, config):
    ...
    if _is_first_commit(original["before"]):
        link = "{0}/commit/{1}".format(
            original["project"]["web_url"], original["after"]
        )
    else:
    ...
    line.append(
        "{PINK}{0}{RESET}".format(
            GitlabHook.shorten(link), **IncomingHookService.colors
        )
    )
...
def _create_commit_summary(project_name, j, config):
    ...
    for commit in original["commits"]:
        ...
        if author:
            line.append(
                "{ORANGE}{author}{RESET}".format(
                    author=author, **IncomingHookService.colors
                )
            )
...
class GitlabHook(EnvironmentMixin, IncomingHookService):
    SERVICE_NAME = "Gitlab"
    SERVICE_ID = 90
    ...
    def _handle_issue_comment(cls, user, request, hook, json):
        fmt_string = (
            "{RESET}[{BLUE}{name}{RESET}] {ORANGE}{who}{RESET} commented on "
            "issue {GREEN}#{num}{RESET}: {title} - {PINK}{url}{RESET}"
        )
    ...
    def _handle_pipeline(cls, user, request, hook, json):
        fmt_string = (
            "{RESET}[{BLUE}{name}{RESET}] Pipeline {GREEN}#{num}{RESET}: "
            "{status_color}{status}{RESET} - {PINK}{url}{RESET}"
        )

        status_color = IncomingHookService.colors["GREEN"]
        if json["object_attributes"]["status"].lower() != "success":
            status_color = IncomingHookService.colors["RED"]
        link = "{0}/pipelines/{1}".format(
            json["project"]["web_url"], json["object_attributes"]["id"]
        )
    ...
    def _handle_build(cls, user, request, hook, json):
        fmt_string = (
            "{RESET}[{BLUE}{name}{RESET}] Build {GREEN}#{num}{RESET}: "
            "{status_color}{status}{RESET} - {PINK}{url}{RESET}"
        )
        ...
        link = "u{0}/builds/{1}".format(
            json["repository"]["homepage"], json["build_id"]
        )
        ...
        if j["tag"]:
            if _is_first_commit(original["after"]):
                if not is_event_allowed(config, "delete", "tag"):
                    return ""
                line.append("deleted tag")
            else:
                if not is_event_allowed(config, "create", "tag"):
                    return ""
                line.append(
                    "tagged {GREEN}{sha}{RESET} as".format(
                        sha=original["after"], **IncomingHookService.colors
                    )
                )
            ...
            if _is_first_commit(original["after"]):
                if not is_event_allowed(config, "delete", "branch"):
                    return ""
                line.append("deleted branch")
            else:
                if not is_event_allowed(config, "create", "branch"):
                    return ""
                line.append("created branch")
